...
<div class="space-y-8 overflow-y-auto pr-2 pb-20">
	...
	{:else if graphState.selectedSymbol}
		...
		<div class="space-y-6 border-t border-white/5 pt-4">
			{#if graphState.isImpactLoading || hasFetchedImpact}
				<section>
					<h3
					...
					{#if graphState.isImpactLoading}
						<div class="flex items-center gap-2 px-3 py-2 text-xs text-text-secondary opacity-50">
							<Loader2Icon class="h-3 w-3 animate-spin" />
							Tracing dependencies...
						</div>
					{:else if graphState.impact.length > 0}
						<div class="space-y-2">
					...
					{:else}
						<p class="px-3 text-[10px] text-text-secondary italic opacity-50">
							No direct or indirect impact detected beyond this symbol.
						</p>
					{/if}
				</section>
			{:else}
				<button
					onclick={() => {
						hasFetchedImpact = true;
						graphState.fetchImpact();
					}}
